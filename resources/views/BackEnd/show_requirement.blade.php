...
<tr>
    <td>{{ $num++ }}</td>
                                                ...
                                                <td>{{ $requirementData['created_at'] }}</td>
                                                <td>
                                                    <span class="badge @if($requirementData['status'] == 1) badge-success @else bg-danger @endif">@if($requirementData['status'] == 1) Active @else Block @endif</span>
                                                </td>
                                                <td class="text-center">
                                                    <div class="row">
                                                        <div class="col-sm-6 text-right"><a href="{{ route("admin_show_requirement") }}/edit/{{ $requirementData['id'] }}"
                                                                class="btn btn-primary btn-sm" title="Edit"><i
                                                                    class="far fa-edit"></i> Edit</a></div>
                                                        <div class="col-sm-6 text-left">
                                                            <a href="{{ route('admin_show_requirement') }}/delete/{{ $requirementData['id'] }}" onclick="return confirm('Can you Delete this Data !');"
